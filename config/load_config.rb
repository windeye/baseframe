...
AppConfig ||= Configurate::Settings.create do
  ...
  unless heroku? || rails_env == "test" || File.exists?(config_dir.join("baseframe.yml"))
    $stderr.puts "FATAL: Configuration not found. Copy over baseframe.yml.example"
    $stderr.puts "       to baseframe.yml and edit it to your needs."
    Process.exit(1)
  end
  
  add_provider Configurate::Provider::YAML,
               config_dir.join("baseframe.yml"),
...
if rails_env == "production"  &&
    ...
    $stderr.puts "FATAL: Diaspora doesn't know where your certificate authorities are. Please ensure they are set to a valid path in baseframe.yml"
    Process.exit(1)
  end
end
